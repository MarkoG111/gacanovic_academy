                                ...
                                <div class="shop-top-bar with-sidebar ">
                                    ...
                                    <div class="product-search">
                                        ...
                                        <footer>
                                            <form class="example" action="">
                                                <input type="text" placeholder="Search For Courses" aria-label="true" name="search" class="form-control" value="{{ $search }}">
                                            </form>
                                        </footer>
                                    </div>
                                    <div class="product-showing">
                                        ...
                                        <select name="showing" id="showing" class="form-control">
                                            @php
                                                $show = [ ['value' => '4'], ['value' => '6',], ['value' => '9',],['value' => '12',],['value' => '15'] ]
                                            @endphp
                                            @foreach ($show as $s)
                                                @if ($s['value'] == $showing)
                                    ...
                                    <div class="">
                                        <p>Pages: {{ $courses->currentPage() }} of {{ $courses->lastPage() }}</p>
                                ...
                                @foreach ($topics as $topic)
                                    <div class="col-12 mb-15 position-relative">
                                        @if (in_array($topic->id_topic, $topicChb ?? []))
                                            <input type="checkbox" name="topic[]" id="remember_me" value="{{ $topic->id_topic }}" checked />
                                            <label for="remember_me">{{ $topic->topic_name }}</label>
                                        @else
                                            <input type="checkbox" name="topic[]" id="remember_me" value="{{ $topic->id_topic }}" />
                                            <label for="remember_me">{{ $topic->topic_name }}</label>
                                        @endif
                                    </div>
                                @endforeach
                                ...
                                @foreach ($categories as $c)
                                    <div class="col-12 mb-15 position-relative">
                                        @if (in_array($c->id_category, $categoriesChb ?? []))
                                            <input type="checkbox" name="categories[]" id="remember_me" checked="checked" value="{{ $c->id_category }}">
                                            <label for="remember_me">{{ $c->category_name }}</label>
                                        @else
                                            <input type="checkbox" name="categories[]" id="remember_me" value="{{ $c->id_category }}">
                                            <label for="remember_me">{{ $c->category_name }}</label>
                                        @endif
                                    </div>
                                @endforeach
